...
app.get('/getSentinel', function (req,res){
   ...
   promObj['coordinates'] = req.query.data;
    ...
    request(url_search + 'footprint:"Intersects(POLYGON(('+promObj.coordinates+')))" AND platformname:Sentinel-2 AND ingestiondate:[NOW-1MONTH TO NOW] ' + '&rows=100' + '&orderby=beginposition desc&format=json',{auth: auth},function(error, response, body) {
            console.log(body);
            res.send(body);
...
function downloadSentinelSync(promObj){
        try {
            ...
            if (process.platform === "win32") {
                ...
                console.log("executing:", 'sh downloadProducts.sh ' + urls + ' ' + names);
                child = exec('sh downloadProducts.sh ' + urls + ' ' + names);
